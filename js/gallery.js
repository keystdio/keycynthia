...
function loadImages() {
    s3.listObjects({Bucket: imageBucketName}, function(error, data) {
        if (error) {
            ...
            for (var i = 0; i < data.Contents.length; i++) {
                ...
                loadSingleImage(currImageName);
            }
        }
    });
}

function loadSingleImage(currImageName) {
    s3.getSignedUrl('getObject', {Bucket: imageBucketName, Key: currImageName}, function(err, url) {
        if (err) {
            console.error(err);
        } else {
